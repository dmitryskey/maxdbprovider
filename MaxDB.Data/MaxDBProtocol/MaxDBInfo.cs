//-----------------------------------------------------------------------------------------------
// <copyright file="MaxDBInfo.cs" company="2005-2021 Dmitry S. Kataev, 2002-2003 SAP AG">
...
namespace MaxDB.Data.MaxDBProtocol
{
    ...
    /// </summary>
    internal class MaxDBParseInfo : IMaxDBParseInfo
    {
        ...
        internal MaxDBParseInfo(MaxDBConnection connection, string sqlCmd, int functionCode)
        {
        ...
        public bool IsValid => this.iSessionId == this.DbConnection.Comm.SessionID;

        /// <summary>
        /// Gets the information about parameters in sql statement.
        /// </summary>
        public IDBTechTranslator[] ParamInfo => this.ParamInfos;

        /// <summary>
        /// Gets the information about ptable columns.
        /// </summary>
        public IDBTechTranslator[] ColumnInfo => this.ColumnInfos;
        ...
        /// </summary>
        public bool VarDataInput { get; set; }

        /// <summary>
        /// Gets or sets DB stored procedure parameters info.
        /// </summary>
        public IDBProcParameterInfo[] ProcParamInfos { get; set; }

        /// <summary>
        /// Gets or sets the parameter DB Tech translators.
        /// </summary>
        public IDBTechTranslator[] ParamInfos { get; set; }
        ...
        /// </summary>
        public IDBTechTranslator[] ColumnInfos { get; set; }
        ...
        {
            if (this.DbConnection != null && this.DbConnection.Comm != null)
            {
                if (this.ParseID != null)
                {
                    this.DbConnection.Comm.DropParseID(this.ParseID);
                    this.ParseID = null;
                }

                if (this.byMassParseId != null)
                {
                    this.DbConnection.Comm.DropParseID(this.byMassParseId);
        ...
        public void SetShortInfosAndColumnNames(IDBTechTranslator[] shortInfo, string[] columnNames)
        {
            ...
            {
                this.ParamInfos = this.ColumnInfos = Array.Empty<IDBTechTranslator>();
                return;
            }
        ...
        public void SetMetaData(IDBTechTranslator[] info, string[] colName)
        {
            int colCount = info.Length;
            IDBTechTranslator currentInfo;
            string currentName;
        ...
        private void DescribeProcedureCall()
        {
            ...
            using (MaxDBCommand cmd = new MaxDBCommand(sql, this.DbConnection))
            {
                ...
                if (!rs.Read())
                {
                    this.ProcParamInfos = Array.Empty<IDBProcParameterInfo>();
                    rs.Close();
                    return;
                }

                var parameterInfos = new List<IDBProcParameterInfo>();
                IDBProcParameterInfo currentInfo = null;
                int currentIndex = 0;
                do
                {
    ...
    /// </summary>
    internal class StructureElement : IStructureElement
    {
        /// <summary>
        ...
        internal StructureElement(string typeName, string codeType, int length, int precision, int asciiOffset, int unicodeOffset)
        {
    ...
    /// </summary>
    internal class DBProcParameterInfo : IDBProcParameterInfo
    {
        /// <summary>
        /// ABAP table code.
        /// </summary>
        ...
        public IStructureElement this[int index] => this.lstTypeElements[index];
    ...
    internal class FetchInfo
    {
        ...
        private IDBTechTranslator[] mColumnInfo;            // short info of all columns
        ...
        public FetchInfo(MaxDBConnection connection, string cursorName, IDBTechTranslator[] infos, string[] columnNames)
        {
            this.dbConnection = connection;
        ...
        public IMaxDBReplyPacket ExecFetchNext()
        {
            if (this.mColumnInfo == null)
            {
                this.Describe();
            ...
            var request = this.dbConnection.Comm.GetRequestPacket();
            byte currentSQLMode = request.SwitchSqlMode((byte)SqlMode.Internal);
            request.InitDbsCommand(cmd, this.dbConnection.AutoCommit);
            ...
            try
            {
                return this.dbConnection.Comm.Execute(this.dbConnection.ConnArgs, request, this, GCMode.DELAYED);
            }
            finally
            {
        ...
        public IDBTechTranslator GetColumnInfo(int index)
        {
            if (this.mColumnInfo == null)
            {
                this.Describe();
        ...
        private void SetMetaData(IDBTechTranslator[] info, string[] colName)
        {
            int colCount = info.Length;
            IDBTechTranslator currentInfo;
            int currentFieldEnd;
        ...
        private void Describe()
        {
            ...
            try
            {
                request.InitDbsCommand($"DESCRIBE \"{this.strCursorName}\"", false);

                //// >>> SQL TRACE
                this.dbConnection.mLogger.SqlTrace(DateTime.Now, "::DESCRIBE CURSOR " + this.strCursorName);
                //// <<< SQL TRACE

                var reply = this.dbConnection.Comm.Execute(this.dbConnection.ConnArgs, request, this, GCMode.ALLOWED);
                ...
                for (int i = 0; i < reply.PartCount; i++)
                {
                    ...
                    {
                        infos = reply.ParseShortFields(this.dbConnection.Comm.ConnStrBuilder.SpaceOption, false, null, false);
                    }
                    else if (partType == PartKind.VardataShortInfo)
                    {
                        infos = reply.ParseShortFields(this.dbConnection.Comm.ConnStrBuilder.SpaceOption, false, null, true);
                    }
                }
