//-----------------------------------------------------------------------------------------------
// <copyright file="MaxDBPacket.cs" company="2005-2021 Dmitry S. Kataev, 2002-2003 SAP AG">
...
namespace MaxDB.Data.MaxDBProtocol
{
    ...
    /// </summary>
    internal class MaxDBPacket : ByteArray, IMaxDBPacket
    {
    ...
    /// <summary>
    /// MaxDB Request Packet.
    /// </summary>
    internal class MaxDBRequestPacket : MaxDBPacket, IMaxDBRequestPacket
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="MaxDBRequestPacket"/> class.
        /// </summary>
        ...
        /// </summary>
        /// <param name="data"></param>
        /// <param name="clientEncoding"></param>
        /// <param name="appID"></param>
        /// <param name="appVersion"></param>
        ...
        public virtual void InitDbsCommand(string command, bool autoCommit) => this.InitDbsCommand(command, true, autoCommit);

        public virtual bool InitDbsCommand(string command, bool reset, bool autoCommit, bool unicode = false)
        {
            if (command == null)
            {
        ...
        {
            this.InitDbsCommand("CONNECT " + user + "  AUTHENTICATION", false);
            this.ClosePart();
        ...
        public IMaxDBDataPart InitGetValue(bool autoCommit)
        {
            this.Reset();
        ...
        public IMaxDBDataPart InitPutValue(bool autoCommit)
        {
            this.Reset();
        ...
        public IMaxDBDataPart NewDataPart(bool varData) => varData ? this.NewVarDataPart() : this.NewDataPart();

        private IMaxDBDataPart NewDataPart() => this.NewDataPart(PartKind.Data);

        public IMaxDBDataPart NewDataPart(byte partKind)
        {
    ...
    internal class MaxDBReplyPacket : MaxDBPacket, IMaxDBReplyPacket
    {
        ...
        public IDataPartVariable VarDataPart
        {
            get
            {
                try
        ...
        public IDBTechTranslator[] ParseShortFields(bool spaceoption, bool isDBProcedure, IDBProcParameterInfo[] procParameters, bool isVardata)
        {
            int columnCount;
            IDBTechTranslator[] result;
            ...
            for (int i = 0; i < columnCount; ++i)
            {
                IDBProcParameterInfo info = null;
        ...
        protected virtual IDBTechTranslator GetTranslator(int mode, int ioType, int dataType, int frac, int len,
            int ioLen, int bufpos, bool spaceoption, bool isDBProcedure, IDBProcParameterInfo procParamInfo)
        {
            switch (dataType)
            {
                case DataType.CHA:
    ...
    internal class MaxDBUnicodeReplyPacket : MaxDBReplyPacket
    {
        ...
        protected override IDBTechTranslator GetTranslator(int mode, int ioType, int dataType, int frac, int len,
                  int ioLen, int bufpos, bool spaceoption, bool isDBProcedure, IDBProcParameterInfo procParamInfo)
        {
            switch (dataType)
            {
