...
namespace MaxDB.Data
{
    ...
    {
        private const int MaxParseAgainCnt = 10;
        ...
        public MaxDBCommand(string cmdText) => this.CommandText = cmdText;
        ...
        public MaxDBCommand(string cmdText, MaxDBConnection connection, MaxDBTransaction transaction)
        ...
        {
            if (this.Connection == null || this.Connection.Comm == null)
            {
        ...
        {
            this.AssertOpen();
            this.strCursorName = this.Connection.Comm.NextCursorName;
            ...
            {
                this.Connection.mLogger.SqlTrace(dt, "::EXECUTE BATCH " + this.strCursorName);
                this.Connection.mLogger.SqlTrace(dt, "BATCH SIZE " + batchParams.Length);
            }
            //////<<< SQL TRACE
            ...
            try
            {
                ...
                while ((inputCursor < count) && noError)
                {
                    ...
                    try
                    {
                        replyPacket = this.Connection.Comm.Execute(this.Connection.ConnArgs, requestPacket, this, GCMode.DELAYED);
                    }
        ...
        private IMaxDBReplyPacket SendCommand(IMaxDBRequestPacket requestPacket, string sqlCmd, int gcFlags, bool parseAgain)
        {
            ...
            return this.Connection.Comm.Execute(this.Connection.ConnArgs, requestPacket, false, true, this, gcFlags);
        }

        private IMaxDBReplyPacket SendSqlCommand(string sql, bool parseAgain)
        {
            try
            {
                return this.SendCommand(this.Connection.Comm.GetRequestPacket(), sql, GCMode.ALLOWED, parseAgain);
            }
        ...
        internal bool Execute(CommandBehavior behavior)
        {
            ...
            return this.Execute(MaxParseAgainCnt, behavior);
        }
        ...
        internal bool Execute(int afterParseAgain, CommandBehavior behavior)
        {
            ...
            if (this.Connection.mLogger.TraceSQL)
            {
                this.Connection.mLogger.SqlTrace(dt, "::EXECUTE " + this.strCursorName);
            }
            //////<<< SQL TRACE
            ...
            try
            {
                ...
                try
                {
                    ...
                    // DB Procedures.
                    replyPacket = this.Connection.Comm.Execute(this.Connection.ConnArgs, requestPacket, this, this.lstInputProcedureLongs == null ? GCMode.ALLOWED : GCMode.NONE);
                }
                catch (MaxDBException ex)
                {
                    if (ex.ErrorCode == -8 && afterParseAgain > 0)
                    {
                        ...
                        if (this.Connection.mLogger.TraceSQL)
                        {
                            this.Connection.mLogger.SqlTrace(dt, "PARSE AGAIN");
                        }
                        //////<<< SQL TRACE

                        ResetPutValues(this.lstInputLongs);
                        this.Reparse();
                        this.Connection.Comm.FreeRequestPacket(requestPacket);
                        afterParseAgain--;
            ...
            {
                if (this.Connection.Comm.IsInTransaction)
                {
                    throw;
                }
                else
        ...
        private void FillInputParameters(MaxDBParameterCollection cmdParams, ref object[] inputArgs)
        {
            ...
            if (this.Connection.mLogger.TraceSQL && this.mParseInfo.InputCount > 0)
            {
                ...
                this.Connection.mLogger.SqlTraceDataHeader(dt);
            }
            ////<<< SQL TRACE
            ...
            for (int i = 0; i < this.mParseInfo.ParamInfo.Length; i++)
            {
                ...
                if (this.Connection.mLogger.TraceSQL)
                {
                    ...
                    this.Connection.mLogger.SqlTrace(dt, sbOut.ToString());
                }
                ////<<< SQL TRACE
                    ...
                    switch (param.dbType)
                    {
                        ...
                        case MaxDBType.LongUni:
                            if (param.objInputValue != null && param.objInputValue.ToString().Length == 0
                                && this.Connection.SqlMode == SqlMode.Oracle)
                            {
        ...
        private void FillOutputParameters(IMaxDBReplyPacket replyPacket, ref MaxDBParameterCollection cmd_params)
        {
            DateTime dt = DateTime.Now;
        ...
        private bool ParseResult(IMaxDBReplyPacket replyPacket, IDBTechTranslator[] infos, string[] columnNames, CommandBehavior behavior)
        {
            ...
            for (int i = 0; i < replyPacket.PartCount; i++)
            {
                ...
                switch (replyPacket.PartType)
                {
                    ...
                    case PartKind.ShortInfo:
                        if (infos == null)
                        {
                            infos = replyPacket.ParseShortFields(this.Connection.Comm.ConnStrBuilder.SpaceOption, false, null, false);
                        }
                    ...
                    case PartKind.VardataShortInfo:
                        if (infos == null)
                        {
                            infos = replyPacket.ParseShortFields(this.Connection.Comm.ConnStrBuilder.SpaceOption, false, null, true);
                        }
                    ...
                    case PartKind.ResultCount:
                        ...
                        if (!isQuery)
                        {
                            ...
                            if (this.Connection.mLogger.TraceSQL)
                            {
                                this.Connection.mLogger.SqlTrace(dt, "RESULT COUNT: " + this.iRowsAffected.ToString(CultureInfo.InvariantCulture));
                            }
                            ////<<< SQL TRACE
                        }
                    ...
                    case PartKind.ErrorText:
                        if (replyPacket.ReturnCode == 100)
                        {
                            ...
                            if (this.Connection.mLogger.TraceSQL)
                            {
                                this.Connection.mLogger.SqlTrace(dt, "*** ROW NOT FOUND ***");
                            }
                            ////<<< SQL TRACE
                            this.iRowsAffected = -1;
                            rowNotFound = true;
        ...
        private MaxDBDataReader CreateDataReader(IDBTechTranslator[] infos, string[] columnNames, bool rowNotFound, CommandBehavior behavior, IMaxDBReplyPacket reply)
        {
            MaxDBDataReader reader;

            try
            {
                reader = new MaxDBDataReader(this.Connection, new FetchInfo(this.Connection, this.strCursorName, infos, columnNames), this, (behavior & CommandBehavior.SingleRow) != 0 ? 1 : 0, reply);
            }
            catch (MaxDBException ex)
            {
        ...
        private IMaxDBParseInfo DoParse(string sql, bool parseAgain)
        {
            if (sql == null)
            {
            ...
            IMaxDBReplyPacket replyPacket;
            IMaxDBParseInfo result = null;
            var cache = this.Connection.Comm.ParseCache;
            ...
            else if (cache != null)
            {
                ...
                ////>>> SQL TRACE
                if (this.Connection.mLogger.TraceSQL && result != null)
                {
                    this.Connection.mLogger.SqlTrace(DateTime.Now, "CACHED PARSE ID: 0x" + Consts.ToHexString(result.ParseID));
                }
                ////<<< SQL TRACE
            }
            ...
            if (result == null || parseAgain)
            {
                ...
                if (this.Connection.mLogger.TraceSQL)
                {
                    this.Connection.mLogger.SqlTrace(dt, "::PARSE " + this.strCursorName);
                    this.Connection.mLogger.SqlTrace(dt, "SQL COMMAND: " + sql);
                }
                ////<<< SQL TRACE
                ...
                if (!parseAgain)
                {
                    result = new MaxDBParseInfo(this.Connection, sql, replyPacket.FuncCode);
                }

                replyPacket.ClearPartOffset();
                IDBTechTranslator[] shortInfos = null;
                for (int i = 0; i < replyPacket.PartCount; i++)
                {
                    replyPacket.NextPart();
                    switch (replyPacket.PartType)
                    {
                        case PartKind.ParseId:
                            ...
                            if (this.Connection.mLogger.TraceSQL)
                            {
                                this.Connection.mLogger.SqlTrace(dt, "PARSE ID: 0x" + Consts.ToHexString(result.ParseID));
                            }
                            ////<<< SQL TRACE
                            break;
                        case PartKind.ShortInfo:
                            shortInfos = replyPacket.ParseShortFields(this.Connection.Comm.ConnStrBuilder.SpaceOption, result.IsDBProc, result.ProcParamInfos, false);
                            break;
                        case PartKind.VardataShortInfo:
                            result.VarDataInput = true;
                            shortInfos = replyPacket.ParseShortFields(this.Connection.Comm.ConnStrBuilder.SpaceOption, result.IsDBProc, result.ProcParamInfos, true);
                            break;
                        case PartKind.ResultTableName:
            ...
            if (this.Connection.mLogger.TraceSQL)
            {
                this.Connection.mLogger.SqlTraceParseInfo(DateTime.Now, result);
            }
            ////<<< SQL TRACE

            result.DbConnection = this.Connection;
            return result;
        }
        ...
        private IMaxDBReplyPacket ProcessProcedureStreams(IMaxDBReplyPacket packet)
        {
        ...
        private void HandleStreamsForPutValue(IMaxDBReplyPacket replyPacket)
        {
            ...
            while (!lastStream.AtEnd)
            {
                ...
                requestPacket = this.Connection.Comm.GetRequestPacket();
                dataPart = requestPacket.InitPutValue(this.Connection.AutoCommit);
                ...
                if (requiresTrailingPacket && !this.Canceled)
                {
                    ...
                    lastStream.MarkAsLast(dataPart);
                    dataPart.Close();
                    this.Connection.Comm.Execute(this.Connection.ConnArgs, requestPacket, this, GCMode.DELAYED);
                }
            }
        ...
        private void GetChangedPutValueDescriptors(IMaxDBReplyPacket replyPacket)
        {
        ...
        private IDBTechTranslator FindColumnInfo(int colIndex)
        {
            try
            {
        ...
        IMaxDBConnection ISqlParameterController.Connection => this.Connection;

        IByteArray ISqlParameterController.ReplyData => this.baReplyMemory;
        ...
        {
            var clone = new MaxDBCommand(this.CommandText, this.Connection, this.Transaction);
        ...
        public override void Cancel()
        {
            this.AssertOpen();
            this.Canceled = true;
            this.Connection.Comm.Cancel(this);
        }
        ...
        protected override DbConnection DbConnection
        {
            ...
            set => this.Connection = value as MaxDBConnection;
        }

        /// <summary>
        /// Gets or sets the <see cref="DbConnection"/> used by this instance of the <b>MaxDBCommand</b>.
        /// </summary>
