//-----------------------------------------------------------------------------------------------
// <copyright file="MaxDBConnection.cs" company="2005-2021 Dmitry S. Kataev, 2002-2003 SAP AG">
...
namespace MaxDB.Data
{
    ...
    /// </summary>
    public sealed class MaxDBConnection : DbConnection, IMaxDBConnection
    {
        /// <summary>
        /// Gets or sets MaxDB communication object.
        /// </summary>
        IMaxDBComm IMaxDBConnection.Comm
        {
            get; set;
        }
        ...
        /// </summary>
        public Encoding DatabaseEncoding => this.Comm.Encoding;

        private Encoding userAsciiEncoding;
        internal MaxDBConnectionStringBuilder ConnStrBuilder;
        private string strConnection;

        ConnectArgs IMaxDBConnection.ConnArgs { get; set; }

        internal ConnectArgs ConnArgs
        {
            get => (this as IMaxDBConnection).ConnArgs;
            private set => (this as IMaxDBConnection).ConnArgs = value;
        }
            ...
            : this()
        {
            ...
            this.ConnStrBuilder = new MaxDBConnectionStringBuilder(connectionString);
        ...
        private void SetConnectionParameters()
        {
            ...
            args.dbname = this.ConnStrBuilder.InitialCatalog;
            args.username = this.ConnStrBuilder.UserId;
            ...
            if (this.ConnStrBuilder.CodePage > 0)
            {
                ...
                this.UserAsciiEncoding = Encoding.GetEncoding(this.ConnStrBuilder.CodePage);
            }
        }
        ...
        {
            get => this.ConnStrBuilder.Mode;

            set
            {
                this.ConnStrBuilder.Mode = value;
                if (this.Comm != null)
                {
                    this.Comm.ConnStrBuilder.Mode = value;
                }
            }
        }
        ...
        public override string ServerVersion
        {
            get
            {
                int version = this.Comm.KernelVersion;
        ...
        internal int KernelVersion => this.Comm.KernelVersion;
        ...
        {
            if (this.Comm.IsolationLevel != level)
            {
                this.AssertOpen();
                string cmd = "SET ISOLATION LEVEL " + MapIsolationLevel(level).ToString(CultureInfo.InvariantCulture);
                var requestPacket = this.Comm.GetRequestPacket();
                byte oldMode = requestPacket.SwitchSqlMode((byte)SqlMode.Internal);
                requestPacket.InitDbsCommand(cmd, this.Comm.AutoCommit);
                try
                {
                    this.Comm.Execute(this.ConnArgs, requestPacket, this, GCMode.ALLOWED);
                }
                ...
                this.Comm.IsolationLevel = level;
            }
        }
        ...
        public override void Close()
        {
            if (this.State == ConnectionState.Open)
            {
                ...
                if (this.Comm != null)
                {
                    if (this.ConnStrBuilder.Pooling)
                    {
                        MaxDBConnectionPool.ReleaseEntry(this);
                    }
                    else
                    {
                        this.Comm.Close(true, true);
                        this.Comm.Dispose();
                    }
        ...
        public override string ConnectionString
        {
            ...
            set
            {
                ...
                this.ConnStrBuilder = new MaxDBConnectionStringBuilder(value);
                this.SetConnectionParameters();
            }
        }

        /// <summary>
        /// Gets the connection time-out value set in the connection
        /// string. Zero indicates an indefinite time-out period.
        /// </summary>
        public override int ConnectionTimeout => this.ConnStrBuilder.Timeout;
        ...
        internal bool Ping(IMaxDBComm communication)
        {
            var oldMode = communication.ConnStrBuilder.Mode;
            var oldComm = this.Comm;
            this.Comm = communication;
            if (this.Comm.ConnStrBuilder.Mode != SqlMode.Internal)
            {
                this.Comm.ConnStrBuilder.Mode = SqlMode.Internal;
            }
        ...
        private DataTable ExecuteInternalQuery(string sql, string table, MaxDBParameterCollection parameters = null)
        {
            ...
            {
                var oldMode = this.Comm.ConnStrBuilder.Mode;
                if (oldMode != SqlMode.Internal)
                {
                    this.Comm.ConnStrBuilder.Mode = SqlMode.Internal;
                }
                ...
                {
                    if (this.Comm.ConnStrBuilder.Mode != oldMode)
                    {
                        this.Comm.ConnStrBuilder.Mode = oldMode;
                    }
                }
        ...
        /// </summary>
        public override string Database => this.ConnArgs.dbname;

        /// <summary>
        /// Gets Data source address or IP.
        /// </summary>
        public override string DataSource => this.ConnArgs.host;
        ...
        public override void Open()
        {
            if (this.ConnStrBuilder.Pooling)
            {
                this.Comm = MaxDBConnectionPool.GetPoolEntry(this, this.mLogger);
            }
            else
            {
                this.Comm = new MaxDBComm(this.mLogger)
                {
                    ConnStrBuilder = this.ConnStrBuilder,
                };

                this.Comm.Open(this.ConnArgs);
            }
        }
        ...
        /// </summary>
        public override ConnectionState State => this.Comm != null && this.Comm.SessionID >= 0 ? ConnectionState.Open : ConnectionState.Closed;
